...
static constexpr size_t total_iterations = 1'000'000;
static constexpr char request_endpoint[] =
  "https://service.example.com/v1/resources/0123456789abcdef";
static constexpr char payload[] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

/**
 * Measures JSON throughput with named arguments, including the backend's reusable named-argument
 * storage. The longer field values make allocation and capacity-reuse regressions visible.
 */
int main()
{
  ...
  std::shared_ptr<quill::Sink> file_sink = quill::Frontend::create_or_get_sink<quill::JsonFileSink>(
    "quill_backend_throughput_json_named_args.log",
    []()
    {
  ...
  LOG_INFO(logger, "preallocate {value}", 0);
  logger->flush_log(0);
  ...
  {
    LOG_INFO(logger,
             "Iteration: {iteration_identifier} endpoint: {request_endpoint} payload: {payload}",
             iteration, request_endpoint, payload);
  }
