...
#include "quill/backend/ProcessUniqueLock.h"
...
public:
  ...
  QUILL_ATTRIBUTE_COLD void run(BackendOptions const& options)
  {
    ...
    _process_unique_lock = std::make_unique<ProcessUniqueLock>(_process_id);
  ...
  QUILL_ATTRIBUTE_COLD void stop() noexcept
  {
    ...
    _process_unique_lock.reset(nullptr);
  }
...
private:
  ...
  std::unique_ptr<ProcessUniqueLock> _process_unique_lock;
