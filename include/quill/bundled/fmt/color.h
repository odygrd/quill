...
class text_style {
 ...
 public:
  ...
  friend FMTQUILL_CONSTEXPR auto operator|(text_style lhs, text_style rhs)
      -> text_style {
    return lhs |= rhs;
  }
 ...
 private:
  FMTQUILL_CONSTEXPR text_style(uint64_t style) noexcept : style_(style) {}
...
template <typename Char> struct ansi_color_escape {
  FMTQUILL_CONSTEXPR ansi_color_escape(color_type text_color,
  ...
  FMTQUILL_CONSTEXPR auto end() const noexcept -> const Char* {
    return buffer + size;
  }
...
FMTQUILL_CONSTEXPR auto make_foreground_color(color_type foreground) noexcept
    -> ansi_color_escape<Char> {
  return ansi_color_escape<Char>(foreground, "\x1b[38;2;");
}

template <typename Char>
FMTQUILL_CONSTEXPR auto make_background_color(color_type background) noexcept
    -> ansi_color_escape<Char> {
...
template <typename T> struct styled_arg : view {
  const T& value;
  text_style style;
  FMTQUILL_CONSTEXPR styled_arg(const T& v, text_style s) : value(v), style(s) {}
};
 ...
 * **Example**:
 *
 *     fmtquill::println(fmtquill::emphasis::bold | fg(fmtquill::color::red),
 *                  "Elapsed time: {0:.2f} seconds", 1.23);
 */
template <typename... T>
...
inline auto format(text_style ts, format_string<T...> fmt, T&&... args)
    -> std::string {
  return fmtquill::vformat(ts, fmt.str, vargs<T...>{{args...}});
}
...
struct formatter<detail::styled_arg<T>, Char> : formatter<T, Char> {
  ...
  FMTQUILL_CONSTEXPR auto format(const detail::styled_arg<T>& arg,
