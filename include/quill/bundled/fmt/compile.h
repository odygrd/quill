...
#  define FMTQUILL_COMPILE(s) FMTQUILL_STRING_IMPL(s, fmtquill::compiled_string)
#else
 ...
 *     std::string s = fmtquill::format("{}"_cf, 42);
 */
...
template <typename Char, typename V, int N> struct spec_field {
  ...
  template <typename OutputIt, typename... T>
  constexpr FMTQUILL_INLINE auto format(OutputIt out, const T&... args) const
      -> OutputIt {
    const auto& vargs =
...
FMTQUILL_INLINE FMTQUILL_CONSTEXPR_STRING auto format(const CompiledFormat& cf,
                                            const T&... args)
...
template <typename OutputIt, typename CompiledFormat, typename... T,
          FMTQUILL_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr FMTQUILL_INLINE auto format_to(OutputIt out, const CompiledFormat& cf,
...
template <typename S, typename... T,
          FMTQUILL_ENABLE_IF(is_compiled_string<S>::value)>
FMTQUILL_INLINE FMTQUILL_CONSTEXPR_STRING auto format(const S&, T&&... args)
  ...
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
    ...
    return fmtquill::format(compiled, std::forward<T>(args)...);
  }
}

template <typename OutputIt, typename S, typename... T,
          FMTQUILL_ENABLE_IF(is_compiled_string<S>::value)>
FMTQUILL_CONSTEXPR auto format_to(OutputIt out, const S&, T&&... args) -> OutputIt {
  constexpr auto compiled = detail::compile<T...>(S());
  if constexpr (std::is_same<remove_cvref_t<decltype(compiled)>,
                             detail::unknown_format>()) {
    ...
    return fmtquill::format_to(out, compiled, std::forward<T>(args)...);
  }
}
#endif
...
auto format_to_n(OutputIt out, size_t n, const S& fmt, T&&... args)
  ...
  fmtquill::format_to(appender(buf), fmt, std::forward<T>(args)...);
  return {buf.out(), buf.count()};
}

template <typename S, typename... T,
          FMTQUILL_ENABLE_IF(is_compiled_string<S>::value)>
FMTQUILL_CONSTEXPR20 auto formatted_size(const S& fmt, T&&... args) -> size_t {
...
template <size_t N> class static_format_result {
 ...
 public:
  template <typename S, typename... T,
            FMTQUILL_ENABLE_IF(is_compiled_string<S>::value)>
  explicit FMTQUILL_CONSTEXPR static_format_result(const S& fmt, T&&... args) {
    *fmtquill::format_to(data, fmt, std::forward<T>(args)...) = '\0';
  }

  FMTQUILL_CONSTEXPR auto str() const -> fmtquill::string_view { return {data, N - 1}; }
  FMTQUILL_CONSTEXPR auto c_str() const -> const char* { return data; }
};
 ...
 * a `fmtquill::string_view` via `str()`.
 *
 * **Example**:
 *
 *     // Produces the static string "42" at compile time.
 *     static constexpr auto result = FMTQUILL_STATIC_FORMAT("{}", 42);
 *     const char* s = result.c_str();
 */
#define FMTQUILL_STATIC_FORMAT(fmt_str, ...)                            \
  fmtquill::static_format_result<                                       \
      fmtquill::formatted_size(FMTQUILL_COMPILE(fmt_str), __VA_ARGS__) + 1>( \
      FMTQUILL_COMPILE(fmt_str), __VA_ARGS__)
