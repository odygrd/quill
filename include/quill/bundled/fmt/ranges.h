...
#else
#  define FMTQUILL_LIFETIMEBOUND
#endif
FMTQUILL_PRAGMA_CLANG(diagnostic error "-Wreturn-stack-address")
...
template <typename T>
FMTQUILL_CONSTEXPR auto range_begin(T&& rng)
    -> decltype(static_cast<T&&>(rng).begin()) {
  return static_cast<T&&>(rng).begin();
}
template <typename T>
FMTQUILL_CONSTEXPR auto range_end(T&& rng) -> decltype(static_cast<T&&>(rng).end()) {
...
struct range_formatter<
 ...
 public:
  ...
  FMTQUILL_CONSTEXPR auto format(R&& range, FormatContext& ctx) const
      -> decltype(ctx.out()) {
    auto out = ctx.out();
...
struct formatter<
 ...
 public:
  ...
  FMTQUILL_CONSTEXPR formatter() {
    if FMTQUILL_CONSTEXPR20 (range_format_kind<R, Char>::value != range_format::set)
      return;
  ...
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto format(range_type& range, FormatContext& ctx) const
      -> decltype(ctx.out()) {
...
struct formatter<
 ...
 public:
  ...
  auto format(range_type& range, FormatContext& ctx) const
      -> decltype(ctx.out()) {
    auto out = ctx.out();
    if FMTQUILL_CONSTEXPR20 (range_format_kind<R, Char>::value ==
                        range_format::debug_string) {
      *out++ = '"';
    }
...
struct join_view : detail::view {
  ...
  FMTQUILL_CONSTEXPR join_view(It b, Sentinel e, basic_string_view<Char> s)
...
struct formatter<join_view<It, Sentinel, Char>, Char> {
 ...
 public:
  ...
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto format(view& value, FormatContext& ctx) const
      -> decltype(ctx.out()) {
    using iter =
...
template <typename Tuple, typename Char> struct tuple_join_view : detail::view {
  ...
  FMTQUILL_CONSTEXPR tuple_join_view(const Tuple& t, basic_string_view<Char> s)
      : tuple(t), sep{s} {}
};
...
struct formatter<tuple_join_view<Tuple, Char>, Char,
  ...
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto format(const tuple_join_view<Tuple, Char>& value,
 ...
 private:
  ...
  FMTQUILL_CONSTEXPR auto do_parse(parse_context<Char>& ctx,
  ...
  template <typename FormatContext>
  FMTQUILL_CONSTEXPR auto do_format(const tuple_join_view<Tuple, Char>&,
                               FormatContext& ctx,
  ...
  FMTQUILL_CONSTEXPR auto do_format(const tuple_join_view<Tuple, Char>& value,
                               FormatContext& ctx,
 ...
 *     // Output: 01, 02, 03
 */
template <typename Range, FMTQUILL_ENABLE_IF(!is_tuple_like<Range>::value)>
FMTQUILL_CONSTEXPR auto join(Range&& r, string_view sep)
 ...
 *     // Output: 1, a
 */
template <typename Tuple, FMTQUILL_ENABLE_IF(is_tuple_like<Tuple>::value)>
FMTQUILL_CONSTEXPR auto join(const Tuple& tuple FMTQUILL_LIFETIMEBOUND, string_view sep)
 ...
 */
template <typename T>
FMTQUILL_DEPRECATED auto join(std::initializer_list<T> list, string_view sep)
