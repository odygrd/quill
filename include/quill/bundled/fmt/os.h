...
FMTQUILL_API std::system_error vwindows_error(int error_code, string_view fmt,
                                         format_args args);
...
struct buffer_size {
  ...
  size_t value = 0;
  FMTQUILL_CONSTEXPR auto operator=(size_t val) const -> buffer_size {
    auto bs = buffer_size();
    bs.value = val;
    return bs;
  }
...
FMTQUILL_INLINE_VARIABLE constexpr auto buffer_size = detail::buffer_size();
