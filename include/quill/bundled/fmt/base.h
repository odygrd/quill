...
#ifdef FMTQUILL_USE_EXCEPTIONS
// Use the provided definition.
#elif defined(__GNUC__) && !defined(__EXCEPTIONS)
#  define FMTQUILL_USE_EXCEPTIONS 0
#elif defined(__clang__) && !defined(__cpp_exceptions)
#  define FMTQUILL_USE_EXCEPTIONS 0
#elif FMTQUILL_MSC_VERSION && !_HAS_EXCEPTIONS
#  define FMTQUILL_USE_EXCEPTIONS 0
#else
#  define FMTQUILL_USE_EXCEPTIONS 1
#endif
...
#ifdef FMTQUILL_NO_UNIQUE_ADDRESS
// Use the provided definition.
#elif FMTQUILL_CPLUSPLUS < 202002L
// Not supported.
#elif FMTQUILL_HAS_CPP_ATTRIBUTE(no_unique_address)
...
#ifdef FMTQUILL_NODISCARD
// Use the provided definition.
#elif FMTQUILL_HAS_CPP17_ATTRIBUTE(nodiscard)
...
// Detect pragmas.
#define FMTQUILL_PRAGMA_IMPL(x) _Pragma(#x)
#if FMTQUILL_GCC_VERSION >= 504 && !defined(__NVCOMPILER)
// Workaround a _Pragma bug https://gcc.gnu.org/bugzilla/show_bug.cgi?id=59884
// and an nvhpc warning: https://github.com/fmtlib/fmt/pull/2582.
#  define FMTQUILL_PRAGMA_GCC(x) FMTQUILL_PRAGMA_IMPL(GCC x)
#else
#  define FMTQUILL_PRAGMA_GCC(x)
#endif
#if FMTQUILL_CLANG_VERSION
#  define FMTQUILL_PRAGMA_CLANG(x) FMTQUILL_PRAGMA_IMPL(clang x)
#else
#  define FMTQUILL_PRAGMA_CLANG(x)
#endif
...
#ifdef _WIN32
#  define FMTQUILL_WIN32 1
#else
#  define FMTQUILL_WIN32 0
#endif

#if !defined(FMTQUILL_HEADER_ONLY) && FMTQUILL_WIN32
...
#ifndef FMTQUILL_OPTIMIZE_SIZE
#  define FMTQUILL_OPTIMIZE_SIZE 0
#endif

// FMTQUILL_BUILTIN_TYPE=0 may result in smaller library size at the cost of higher
// per-call binary size by passing built-in types through the extension API.
#ifndef FMTQUILL_BUILTIN_TYPES
#  define FMTQUILL_BUILTIN_TYPES 1
#endif
...
class basic_specs {
 ...
 public:
  ...
  FMTQUILL_CONSTEXPR void set_upper() { data_ |= uppercase_mask; }

  constexpr auto alt() const -> bool { return (data_ & alternate_mask) != 0; }
  FMTQUILL_CONSTEXPR void set_alt() { data_ |= alternate_mask; }
  FMTQUILL_CONSTEXPR void clear_alt() { data_ &= ~alternate_mask; }

  constexpr auto localized() const -> bool {
    return (data_ & localized_mask) != 0;
  }
  FMTQUILL_CONSTEXPR void set_localized() { data_ |= localized_mask; }
...
template <typename Char = char> class parse_context {
 private:
  ...
  enum { use_constexpr_cast = !FMTQUILL_GCC_VERSION || FMTQUILL_GCC_VERSION >= 1200 };

  FMTQUILL_CONSTEXPR void do_check_arg_id(int arg_id);
...
template <typename T, typename Char,
          FMTQUILL_ENABLE_IF(!is_static_named_arg<T>::value)>
FMTQUILL_CONSTEXPR void init_static_named_arg(named_arg_info<Char>*, int& arg_index,
                                         int&) {
  ++arg_index;
}
template <typename T, typename Char,
          FMTQUILL_ENABLE_IF(is_static_named_arg<T>::value)>
FMTQUILL_CONSTEXPR void init_static_named_arg(named_arg_info<Char>* named_args,
...
template <typename Char> struct type_mapper {
  ...
  template <typename T, typename C = char_t<T>,
            FMTQUILL_ENABLE_IF(!std::is_pointer<T>::value)>
...
template <typename Char> union arg_ref {
  FMTQUILL_CONSTEXPR arg_ref(int idx = 0) : index(idx) {}
  FMTQUILL_CONSTEXPR arg_ref(basic_string_view<Char> n) : name(n) {}
...
FMTQUILL_CONSTEXPR inline auto parse_align(char c) -> align {
  switch (c) {
...
FMTQUILL_CONSTEXPR auto parse_arg_id(const Char* begin, const Char* end,
  ...
  if (FMTQUILL_OPTIMIZE_SIZE > 1 || !is_name_start(c)) {
    report_error("invalid format string");
    return begin;
  }
  auto it = begin;
...
template <typename Char>
FMTQUILL_CONSTEXPR auto parse_width(const Char* begin, const Char* end,
...
template <typename Char, typename Handler>
FMTQUILL_CONSTEXPR FMTQUILL_INLINE auto parse_replacement_field(const Char* begin,
                                                      const Char* end,
                                                      Handler&& handler)
    -> const Char* {
  ++begin;
...
template <typename Char, typename Handler>
FMTQUILL_CONSTEXPR void parse_format_string(basic_string_view<Char> fmt,
                                       Handler&& handler) {
...
FMTQUILL_CONSTEXPR auto invoke_parse(parse_context<Char>& ctx) -> const Char* {
...
class format_string_checker {
 ...
 public:
  template <typename... T>
  FMTQUILL_CONSTEXPR explicit format_string_checker(basic_string_view<Char> fmt,
                                               arg_pack<T...>)
  ...
  FMTQUILL_CONSTEXPR auto on_format_specs(int id, const Char* begin, const Char* end)
      -> const Char* {
...
protected:
 ...
 public:
  ...
  /// Clears this buffer.
  FMTQUILL_CONSTEXPR void clear() { size_ = 0; }
...
class fixed_buffer_traits {
 ...
 public:
  ...
  FMTQUILL_CONSTEXPR auto limit(size_t size) -> size_t {
...
#if !FMTQUILL_BUILTIN_TYPES
#  define FMTQUILL_BUILTIN , monostate
#else
#  define FMTQUILL_BUILTIN
#endif
...
template <typename Context> class value {
 public:
  ...
  constexpr FMTQUILL_INLINE value(signed char x) : int_value(x) {}
  constexpr FMTQUILL_INLINE value(unsigned char x FMTQUILL_BUILTIN) : uint_value(x) {}
  constexpr FMTQUILL_INLINE value(signed short x) : int_value(x) {}
  ...
  constexpr FMTQUILL_INLINE value(float x FMTQUILL_BUILTIN) : float_value(x) {}
  constexpr FMTQUILL_INLINE value(double x FMTQUILL_BUILTIN) : double_value(x) {}
  FMTQUILL_INLINE value(long double x FMTQUILL_BUILTIN) : long_double_value(x) {}
 ...
 private:
  ...
  FMTQUILL_CONSTEXPR value(T& x, custom_tag) {
...
#if FMTQUILL_WIN32
FMTQUILL_API void vprint_mojibake(FILE*, string_view, format_args, bool);
...
template <typename Char>
FMTQUILL_CONSTEXPR void parse_context<Char>::check_dynamic_spec(int arg_id) {
...
template <typename... T> struct fstring {
 ...
 public:
  ...
  template <size_t N>
  FMTQUILL_CONSTEVAL FMTQUILL_ALWAYS_INLINE fstring(const char (&s)[N]) : str(s, N - 1) {
