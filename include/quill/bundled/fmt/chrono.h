...
FMTQUILL_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
  ...
  if FMTQUILL_CONSTEXPR20 (F::digits <= T::digits) {
...
FMTQUILL_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
  ...
  if FMTQUILL_CONSTEXPR20 (F::is_signed && !T::is_signed) {
...
auto safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from,
  ...
  if FMTQUILL_CONSTEXPR20 (Factor::num != 1) {
  ...
  if FMTQUILL_CONSTEXPR20 (Factor::den != 1) {
...
void write_codecvt(codecvt_result<CodeUnit>& out, string_view in,
  ...
  FMTQUILL_PRAGMA_CLANG(diagnostic ignored "-Wdeprecated")
...
FMTQUILL_NORETURN inline void throw_duration_error() {
...
auto duration_cast(std::chrono::duration<FromRep, FromPeriod> from) -> To {
  ...
  if FMTQUILL_CONSTEXPR20 (factor::den != 1) count /= factor::den;
  int ec = 0;
  auto to =
...
FMTQUILL_CONSTEXPR auto parse_chrono_format(const Char* begin, const Char* end,
  ...
  while (ptr != end) {
    ...
    case 'E': {
      ...
      default:  FMTQUILL_THROW(format_error("invalid format"));
      }
      break;
    }
    case 'O':
...
template <typename Derived> struct null_chrono_spec_handler {
  ...
  FMTQUILL_CONSTEXPR void on_year(numeric_system, pad_type) { unsupported(); }
  ...
  FMTQUILL_CONSTEXPR void on_dec_month(numeric_system, pad_type) { unsupported(); }
  ...
  FMTQUILL_CONSTEXPR void on_day_of_year(pad_type) { unsupported(); }
...
class tm_format_checker : public null_chrono_spec_handler<tm_format_checker> {
 ...
 public:
  ...
  FMTQUILL_NORETURN inline void unsupported() {
    FMTQUILL_THROW(format_error("no format"));
  }

  template <typename Char>
  FMTQUILL_CONSTEXPR void on_text(const Char*, const Char*) {}
  FMTQUILL_CONSTEXPR void on_year(numeric_system, pad_type) {}
  ...
  FMTQUILL_CONSTEXPR void on_dec_month(numeric_system, pad_type) {}
  FMTQUILL_CONSTEXPR void on_dec0_week_of_year(numeric_system, pad_type) {}
  FMTQUILL_CONSTEXPR void on_dec1_week_of_year(numeric_system, pad_type) {}
  FMTQUILL_CONSTEXPR void on_iso_week_of_year(numeric_system, pad_type) {}
  FMTQUILL_CONSTEXPR void on_day_of_year(pad_type) {}
  FMTQUILL_CONSTEXPR void on_day_of_month(numeric_system, pad_type) {}
  ...
  FMTQUILL_CONSTEXPR void on_utc_offset(numeric_system) {
    if (!has_timezone_) FMTQUILL_THROW(format_error("no timezone"));
  }
...
inline auto to_nonnegative_int(T value, Int upper) -> Int {
  if (!std::is_unsigned<Int>::value &&
    ...
    FMTQUILL_THROW(format_error("chrono value is out of range"));
  }
...
class tm_writer {
 private:
  ...
  template <typename T, FMTQUILL_ENABLE_IF(has_tm_gmtoff<T>::value)>
  ...
  void format_tz_name(const T& tm) {
    if (!tm.tm_zone) FMTQUILL_THROW(format_error("no timezone"));
...
struct chrono_format_checker : null_chrono_spec_handler<chrono_format_checker> {
  ...
  FMTQUILL_NORETURN inline void unsupported() { FMTQUILL_THROW(format_error("no date")); }
...
auto format_duration_unit(OutputIt out) -> OutputIt {
  ...
  *out++ = '[';
  ...
  if FMTQUILL_CONSTEXPR20 (Period::den != 1) {
    *out++ = '/';
...
template <typename Char> struct formatter<std::tm, Char> {
 ...
 protected:
  ...
  FMTQUILL_CONSTEXPR void set_localized() { specs_.set_localized(); }

  FMTQUILL_CONSTEXPR auto do_parse(parse_context<Char>& ctx, bool has_timezone)
      -> const Char* {
    auto it = ctx.begin(), end = ctx.end();
    if (it == end || *it == '}') return it;
...
struct formatter<sys_time<Duration>, Char> : private formatter<std::tm, Char> {
  ...
  auto format(sys_time<Duration> val, FormatContext& ctx) const
    ...
    if FMTQUILL_CONSTEXPR20 (period::num == 1 && period::den == 1 &&
