...
/***/
TEST_CASE("multi_frontend_threads_iouring_timestamp_order")
{
  static constexpr size_t number_of_messages = 1000;
  static constexpr size_t number_of_threads = 10;
  static constexpr char const* filename = "multi_frontend_threads_iouring_timestamp_order.log";
  ...
  for (size_t i = 0; i < number_of_threads; ++i)
  {
    threads.emplace_back(
      [i]() mutable
      {
        ...
        auto file_sink = Frontend::create_or_get_sink<IOUringFileSink>(
          filename,
          []()
          {
            FileSinkConfig cfg;
