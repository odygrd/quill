...
namespace quill
{
  ...
  private:
    ...
    public:
                  ...
                  detail::StaticLogRecordInfo const& logline_info) const override
      {
        ...
        auto format_buffer = [this, &memory_buffer, timestamp, thread_id, logger_name, logline_info](auto... tuple_args) {
          fmt::format_to(memory_buffer, _fmt_pattern.data(), tuple_args(timestamp, thread_id, logger_name, logline_info)...);
        };
  ...
  public:
    ...
    PatternFormatter()
    {
      ...
      _set_pattern(
        QUILL_STRING("%(ascii_time) [%(thread)] %(filename):%(lineno) %(level_name) %(logger_name) - %(message)"));
    }
                                              ...
                                              Args const&... args) const
  {
    ...
    {
      _pattern_formatter_helper_part_1->format(formatted_buffer, timestamp, thread_id, logger_name.data(), logline_info);
    }
    ...
    {
      _pattern_formatter_helper_part_3->format(formatted_buffer, timestamp, thread_id, logger_name.data(), logline_info);
    }
    ...
    while (pos < len - 1)
    {
      ...
      if (pattern[pos] == '%')
      {
        ...
        if (!part_2_found && pattern[++pos] == '(' && pattern[++pos] == 'm' && pattern[++pos] == 'e' &&
            pattern[++pos] == 's' && pattern[++pos] == 's' && pattern[++pos] == 'a' && pattern[++pos] == 'g' &&
            pattern[++pos] == 'e' && pattern[++pos] == ')')
        {
  ...
  std::unique_ptr<PatternFormatter::FormatterHelperBase> PatternFormatter::_make_formatter_helper(std::string const& format_pattern_part)
  {
    ...
    std::vector<argument_callback_t> const args_callback_collection = _generate_vector_of_callbacks(format_pattern_part);
