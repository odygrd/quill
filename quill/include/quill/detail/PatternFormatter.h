...
private:
   ...
   */
  template <size_t N>
  [[nodiscard]] static std::unique_ptr<FormatterHelperBase> _make_formatter_helper(std::string const& format_pattern_part);
   ...
   */
  [[nodiscard]] static std::vector<argument_callback_t> _generate_vector_of_callbacks(std::string const& pattern);
   ...
   * @return
   */
  [[nodiscard]] static argument_callback_t _select_argument_callback(std::string const& pattern_attr);

  /**
   * Given a part of the pattern modify it to match the expected lib fmt format
   */
  [[nodiscard]] static std::string _generate_fmt_format_string(std::string pattern);
