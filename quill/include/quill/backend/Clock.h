...
public:
  ...
  {
    uint64_t const ts =
            detail::LogSystemManagerSingleton::instance().log_manager().time_since_epoch(detail::rdtsc());
  ...
  QUILL_NODISCARD QUILL_ATTRIBUTE_HOT static time_point to_time_point(RdtscVal rdtsc) noexcept
  {
    return time_point{std::chrono::nanoseconds{
            detail::LogSystemManagerSingleton::instance().log_manager().time_since_epoch(rdtsc.value())}};
  }
};
} // namespace quill