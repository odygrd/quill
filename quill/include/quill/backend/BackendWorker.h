...
public:
   ...
   */
  QUILL_NODISCARD uint32_t thread_id() const noexcept { return _backend_worker_thread_id; }
...
private:
  ...
  QUILL_ATTRIBUTE_HOT void _write_transit_event(TransitEvent const& transit_event) const
  {
    ...
    LogLevel const log_level = transit_event.log_level_override
      ? *transit_event.log_level_override
    ...
    for (auto& handler : transit_event.logger_details->handlers())
    {
      ...
      if (handler->apply_filters(
            transit_event.thread_id, std::chrono::nanoseconds{transit_event.timestamp},
      ...
      for (ThreadContext *thread_context: _active_thread_contexts_cache)
    {
      std::visit(
        [&all_empty](auto& queue)
        {
          ...
          if constexpr ((std::is_same_v<T, UnboundedSPSCQueue>) || (std::is_same_v<T, BoundedSPSCQueue>))
          {
    ...
    for (ThreadContext *thread_context: _active_thread_contexts_cache)
  {
    std::visit(
      [&thread_context, &min_ts, &tc, this](auto& queue)
      {
        ...
        {
          std::byte* read_pos;
          if constexpr (std::is_same_v<T, UnboundedSPSCQueue>)
          {
            read_pos = _read_unbounded_queue(queue, thread_context);
          }
          else
...
/***/
std::byte* BackendWorker::_read_unbounded_queue(UnboundedSPSCQueue& queue, ThreadContext* thread_context) const
{
