...
private:
    ...
    if (total_events == 0)
    {
      ...
      if (all_queues_empty)
      {
        ...
        if (!removed_loggers.empty())
        {
          ...
          for (auto it = std::begin(_pattern_formatters); it != std::end(_pattern_formatters);)
          {
            if (it->expired())
            {
                                                         ...
                                                         ThreadContext* thread_context, uint64_t ts_now)
  {
    ...
    if (!logger_base->pattern_formatter)
    {
      ...
      auto search_it = std::find_if(
        std::begin(_pattern_formatters), std::end(_pattern_formatters),
    ...
    if (transit_event->macro_metadata->event() != MacroMetadata::Event::Flush)
    {
      ...
      if (transit_event->macro_metadata->is_wide_char_format())
      {
        ...
        QUILL_TRY
        {
          fmtquill::vformat_to(std::back_inserter(transit_event->formatted_msg), format_str,
                               fmtquill::basic_format_args(_format_args_store.data(),
                                                           static_cast<int>(_format_args_store.size())));
        }
  #if !defined(QUILL_NO_EXCEPTIONS)
        ...
        if (transit_event->macro_metadata->is_structured_log_template())
        {
              ...
              search != std::cend(_structured_log_templates))
          {
            ...
            QUILL_TRY
            {
              ...
              for (size_t i = 0; i < _format_args_store.size(); ++i)
              {
                fmtquill::vformat_to(std::back_inserter((*transit_event->structured_kvs)[i].second),
                                     "{}", fmtquill::basic_format_args(&_format_args_store[i], 1));
              }
            }
#if !defined(QUILL_NO_EXCEPTIONS)
            QUILL_CATCH(std::exception const& e)
    ...
    _logger_manager.for_each_logger(
      [this](LoggerBase* logger)
      {
        ...
        if (logger->is_valid_logger())
        {
          ...
          {
            auto search_it = std::find_if(std::begin(_active_sinks_cache), std::end(_active_sinks_cache),
  ...
  void _cleanup_invalidated_thread_contexts()
  {
    ...
    auto found_invalid_and_empty_thread_context =
      std::find_if(std::begin(_active_thread_contexts_cache), std::end(_active_thread_contexts_cache),
...
private:
  ...
  std::vector<fmtquill::basic_format_arg<fmtquill::format_context>> _format_args_store; /** Format args tmp storage as member to avoid reallocation */
