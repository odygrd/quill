...
#include "quill/core/TimestampFormatter.h" // for TimestampFormatter
#include "quill/core/Utilities.h"          // for strlength
#include "quill/core/Attributes.h"          // for QUILL_NODISCARD
#include "quill/core/Common.h"              // for Timezone, Timezon...
...
public:
  ...
  ~PatternFormatter() = default;
    ...
    std::vector<std::pair<std::string, std::string>> const &structured_kvs, FormatBuffer const &log_msg)
  {
    ...
    if (_format.empty())
    {
      ...
      // JsonFileHandler if we want to skip formatting the main message
    ...
    if (_is_set_in_pattern[Attribute::CustomTags])
    {
      ...
      _set_arg_val<Attribute::Message>(std::string_view{log_msg.data(), log_msg.size()});
...
private:
  ...
  void _set_pattern(std::string format_pattern)
  {
    ...
    std::tie(_format, _order_index) = _generate_fmt_format_string(
      ...
      "message"_a = "", "custom_tags"_a = "", "structured_keys"_a = "");
  ...
  /***/
  PatternFormatter::Attribute _attribute_from_string(std::string const& attribute_name)
  {
