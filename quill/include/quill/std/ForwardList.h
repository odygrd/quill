...
#endif
{
  static std::forward_list<T, Allocator> decode_arg(std::byte*& buffer)
  {
    // Read the size
    ...
    {
      arg.emplace_front(Decoder<T>::decode_arg(buffer));
    }
    ...
    for (size_t i = 1; i < number_of_elements; ++i)
    {
      ...
      arg.emplace_after(it, Decoder<T>::decode_arg(buffer));
    }
