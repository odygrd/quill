...
public:
    ...
    : _transit_event_buffer(initial_transit_event_buffer_capacity)
  {
    ...
    {
      _spsc_queue.emplace<UnboundedSPSCQueue>(default_queue_capacity, huge_pages);
    }
    else
    {
      _spsc_queue.emplace<BoundedSPSCQueue>(default_queue_capacity, huge_pages);
    }
  }
  ...
  QUILL_NODISCARD_ALWAYS_INLINE_HOT std::variant<std::monostate, UnboundedSPSCQueue, BoundedSPSCQueue> const& spsc_queue_variant() const noexcept
  {
    return _spsc_queue;
  }

  QUILL_NODISCARD_ALWAYS_INLINE_HOT std::variant<std::monostate, UnboundedSPSCQueue, BoundedSPSCQueue>& spsc_queue_variant() noexcept
  {
    return _spsc_queue;
  }
