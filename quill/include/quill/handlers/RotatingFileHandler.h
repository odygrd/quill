...
#include "quill/common/Attributes.h"      // for QUILL_ATTRIBUTE_COLD, QUIL...
...
#include "quill/common/Common.h"          // for QUILL_UNLIKELY
#include "quill/common/FileUtilities.h"   // for append_index_to_filename
#include "quill/common/Os.h"              // for rename_file
#include "quill/common/QuillError.h"      // for QUILL_THROW, QuillError
...
public:
  ...
  RotatingFileHandlerConfig() : _rotation_at_time_daily{_default_rotation_time_daily()} {}
  ...
  QUILL_ATTRIBUTE_COLD void set_rotation_frequency_and_interval(char frequency, uint32_t interval)
  {
    ...
    _rotation_at_time_daily = _default_rotation_time_daily();
  }
  ...
  QUILL_ATTRIBUTE_COLD void set_rotation_time_daily(std::string const& at_time)
  {
    ...
    _rotation_at_time_daily = _parse_at_time_format(at_time);
  }
...
private:
  /***/
  std::pair<std::chrono::hours, std::chrono::minutes> _default_rotation_time_daily() noexcept
  {
  ...
  /***/
  std::pair<std::chrono::hours, std::chrono::minutes> _parse_at_time_format(std::string const& at_time)
  {
    ...
    auto const at_time_tp = std::make_pair(std::chrono::hours{std::stoi(tokens[0])},
...
private:
  ...
  /***/
  uint64_t _calculate_initial_rotation_tp(uint64_t start_time_ns, quill::RotatingFileHandlerConfig const& config)
  {
  ...
  /***/
  uint64_t _calculate_rotation_tp(uint64_t rotation_timestamp_ns, quill::RotatingFileHandlerConfig const& config)
  {
    if (config.rotation_frequency() == quill::RotatingFileHandlerConfig::RotationFrequency::Minutely)
    {
  ...
  /***/
  quill::fs::path _get_filename(quill::fs::path filename, uint32_t index, std::string const& date_time)
  {
    if (!date_time.empty())
    {
