...
public:
   ...
   */
  TimeRotatingFileHandler(detail::std_fs::path const& base_filename, std::string const& mode,
...
private:
  std::queue<detail::std_fs::path> _created_files; /**< We store in a queue the filenames we created, in order to remove_file them if we exceed _backup_count limit */
