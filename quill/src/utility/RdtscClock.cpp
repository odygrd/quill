...
namespace quill::detail
{
  ...
  /***/
  RdtscClock::RdtscClock(std::chrono::nanoseconds resync_internal /* = std::chrono::seconds{100} */)
    : resync_internal_ticks_(static_cast<std::uint64_t>(resync_internal.count() * RdtscTicks::instance().ticks_per_ns())),
      resync_internal_orginal_(resync_internal_ticks_),
  ...
  std::chrono::system_clock::time_point RdtscClock::time_since_epoch(uint64_t rdtsc_value) const noexcept
  {
    ...
    auto const epoch = std::chrono::nanoseconds{base_time_ + static_cast<int64_t>(diff / ticks_per_nanosecond_)};
    ...
    if (diff > resync_internal_ticks_)
    {
      resync();
    }
  ...
  void RdtscClock::resync() const noexcept
  {
    ...
    for (uint8_t attempt = 0; attempt < max_attempts; ++attempt)
    {
      ...
      if (QUILL_LIKELY(end - beg <= 2000))
      {
        ...
        base_tsc_ = end;
        resync_internal_ticks_ = resync_internal_orginal_;
        return;
      }
    }
    ...
    resync_internal_ticks_ = resync_internal_ticks_ * 2;
  }
}; // namespace quill::detail