...
TEST_SUITE_BEGIN("BacktraceRecordStorage");

using namespace quill;
using namespace quill::detail;

class TestRecord final : public RecordBase
{
public:
  explicit TestRecord(uint32_t x, uint32_t y) : x(x), y(y) {}

  std::unique_ptr<RecordBase> clone() const override { return std::make_unique<TestRecord>(*this); }

  size_t size() const noexcept override { return sizeof(*this); }

  void backend_process(BacktraceRecordStorage&, char const*, GetHandlersCallbackT const&,
...
TEST_CASE("retrieve_records_when_empty")
{
  ...
  BacktraceRecordStorage backtrace_record_storage;
  backtrace_record_storage.set_capacity(logger_name, max_messages);

  // Check retrieved records - nothing to process
  uint32_t counter = 0;
  backtrace_record_storage.process(
    logger_name, [&counter](std::string const& thread_id, RecordBase const*) { ++counter; });
...
TEST_CASE("store_clear_records")
{
  ...
  for (uint32_t i = 0; i < 2; ++i)
  {
    ...
    backtrace_record_storage.store(logger_name, thread_id, test_record.clone());
  }

  // Clear them
  backtrace_record_storage.clear(logger_name);
...
TEST_CASE("store_then_change_capacity")
{
  ...
  backtrace_record_storage.set_capacity(logger_name, max_messages * 2);
...
TEST_CASE("store_then_process_no_wrap_around")
{
  ...
  uint32_t counter = 0;
  backtrace_record_storage.process(
    logger_name, [&counter](std::string const& retrieved_thread_id, RecordBase const* retrieved_record) {
