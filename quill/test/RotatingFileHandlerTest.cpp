...
TEST_CASE("rotation_scheme_index_open_mode_write_clean_up_old_files")
{
  ...
  {
    auto rfh = RotatingFileHandler{
      filename,
      []()
      {
        RotatingFileHandlerConfig cfg;
        cfg.set_rotation_max_file_size(1024);
        cfg.set_rotation_naming_scheme(RotatingFileHandlerConfig::RotationNamingScheme::Index);
        cfg.set_open_mode('w');
        return cfg;
      }(),
      FileEventNotifier{}};

    REQUIRE(quill::detail::file_size(filename) == 0);
    REQUIRE(!fs::exists(filename_1));
    REQUIRE(!fs::exists(filename_2));
    REQUIRE(fs::exists(filename_4));
...
TEST_CASE("rotation_scheme_index_open_mode_append")
{
  ...
  remove_file(filename_3);
  remove_file(filename_4);
  remove_file(filename_5);
  remove_file(filename_6);
}
...

  {
    auto rfh = RotatingFileHandler{
      ...
      []()
      {
        RotatingFileHandlerConfig cfg;
        cfg.set_rotation_max_file_size(1024);
        cfg.set_rotation_naming_scheme(RotatingFileHandlerConfig::RotationNamingScheme::Date);
...

  {
    auto rfh = RotatingFileHandler{
      ...
      []()
      {
        RotatingFileHandlerConfig cfg;
        cfg.set_rotation_max_file_size(1024);
        cfg.set_rotation_naming_scheme(RotatingFileHandlerConfig::RotationNamingScheme::DateAndTime);
        cfg.set_timezone(Timezone::GmtTime);
        cfg.set_open_mode('w');
        return cfg;
      }(),
...
TEST_CASE("rotation_scheme_dateandtime_with_backup_limit_overwrite_rolled_files")
{
  ...
  fs::path const filename_2 =
    "rotation_scheme_date_with_backup_limit_overwrite_rolled_files.20230614_053320.1.log";
  fs::path const filename_1 =
    "rotation_scheme_date_with_backup_limit_overwrite_rolled_files.20230614_053320.log";
  fs::path const filename = "rotation_scheme_date_with_backup_limit_overwrite_rolled_files.log";
...
TEST_CASE("rotation_scheme_dateandtime_with_backup_limit_dont_overwrite_rolled_files")
{
  ...
  fs::path const filename_1 =
    "rotation_scheme_date_with_backup_limit_dont_overwrite_rolled_files.20230612_053320.3.log";
  fs::path const filename_2 =
    "rotation_scheme_date_with_backup_limit_dont_overwrite_rolled_files.20230612_053320.2.log";
  fs::path const filename_3 =
    "rotation_scheme_date_with_backup_limit_dont_overwrite_rolled_files.20230612_053320.1.log";
  fs::path const filename_4 =
    "rotation_scheme_date_with_backup_limit_dont_overwrite_rolled_files.20230612_053320.log";
  fs::path const filename =
    "rotation_scheme_date_with_backup_limit_dont_overwrite_rolled_files.log";
