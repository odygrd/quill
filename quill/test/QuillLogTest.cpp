...
public:
  ...
  {
    QUILL_LOG_INFO(_logger, "Test message for test class const");
  }

  /**
   * Use logger in normal function
   */
  void use_logger() { QUILL_LOG_INFO(_logger, "Test message for test class non const"); }
...
TEST_CASE("log_using_rotating_file_handler")
{
  ...
  {
    QUILL_LOG_INFO(rotating_logger, "Hello rotating file log num {}", i);
    QUILL_LOG_INFO(rotating_logger_2, "Hello rotating file log num {}", i);
  }
...
TEST_CASE("log_using_daily_file_handler")
{
  ...
  for (uint32_t i = 0; i < 20; ++i)
  {
    QUILL_LOG_INFO(daily_logger, "Hello daily file log num {}", i);
  }
...
TEST_CASE("log_using_multiple_stdout_formats")
{
  ...
  for (uint32_t i = 0; i < 20; ++i)
  {
    if (i % 2 == 0)
    {
      QUILL_LOG_INFO(quill::get_logger(), "Hello log num {}", i);
    }
    else
    {
      QUILL_LOG_INFO(custom_logger, "Hello log num {}", i);
    }
  }
  ...
  for (uint32_t i = 0; i < 20; ++i)
  {
    if (i % 2 == 0)
    {
      std::string expected_string =
        "QuillLogTest.cpp:345         LOG_INFO      root         - Hello log num " + std::to_string(i);
...
TEST_CASE("log_using_stderr")
{
  ...
  QUILL_LOG_INFO(custom_logger, "Hello log stderr");
  QUILL_LOG_INFO(custom_logger, "Hello log stderr again");
...
TEST_CASE("log_to_multiple_handlers_from_same_logger")
{
  ...
  QUILL_LOG_INFO(custom_logger, "Hello log multiple handlers");
...
TEST_CASE("check_log_arguments_evaluation")
{
  ...
  QUILL_LOG_DEBUG(logger, "Test log arguments {}", arg_str());
  QUILL_LOG_TRACE_L1(logger, "Test log arguments {}", arg_str());
  REQUIRE_EQ(cnt, 0);

  // 2. checks that log arguments are evaluated only once per log statement
  QUILL_LOG_INFO(logger, "Test log arguments {}", arg_str());
  REQUIRE_EQ(cnt, 1);
...
TEST_CASE("log_enums_with_overloaded_insertion_operator")
{
  ...
  QUILL_LOG_INFO(custom_logger, "{},{},{},{},{},{}", Test1, Test2, Test3, EnumClass::Test4,
