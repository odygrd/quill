...
public:
    ...
    static SBE_CONSTEXPR std::uint32_t priceNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_UINT32;
    }

    static SBE_CONSTEXPR std::uint32_t priceMinValue() SBE_NOEXCEPT
    {
        return UINT32_C(0x0);
    }

    static SBE_CONSTEXPR std::uint32_t priceMaxValue() SBE_NOEXCEPT
    {
    ...
    NewOrder &price(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    NewOrder &quantity(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
    ...
    NewOrder &putSymbol(const char *src, const std::uint8_t length)
    {
        std::uint64_t lengthOfLengthField = 1;
        std::uint64_t lengthPosition = sbePosition();
