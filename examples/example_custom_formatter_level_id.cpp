...
/**
 * Logging to a file using a custom formatter pattern using a level id.
 */
...
int main()
{
  ...
  file_handler->set_pattern(
    QUILL_STRING("%(level_id) %(ascii_time) [%(process)] [%(thread)] %(logger_name) - %(message)"), // log recorder format
