...
- The manual backend thread may log, but it must not use paths that wait for the backend to flush its own queue. In particular, avoid ``logger->flush_log()`` and avoid :cpp:func:`LoggerImpl::set_immediate_flush` on that same thread.
...
- you are trying to make Quill single-threaded
